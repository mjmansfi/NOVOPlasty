...
To run with Docker, some extra steps are needed. Assuming `docker` is installed:
1. Download or build the container.
	- Use `docker pull mjmansfi/novoplasty:4.3.1` to download the container from Dockerhub. If this fails, continue to the next bullet point. If it succeeds, proceed to 2.
	- Use the repository's `Dockerfile` to build a local Docker image. After cloning and changing directory into this `git` repository, use `docker build -t [dockerhub_username]/novoplasty:[version] .`, replacing the text in  the square brackets with appropriate arguments.
...
<code>docker run -it --user $(id -u):$(id -g) --volume "${PWD}":/work  [dockerhub_username]/novoplasty:[version] NOVOPlasty.pl -c config.txt</code>
	...
	- If you want to specify absolute paths in `config.txt`, the paths must be real locations within the *container*, not the host. E.g., if `config.txt` specifies that its reads are located at `/path/to/reads/reads_1.fastq`, that path must be mounted within the container by passing it to `--volume`; `docker run --volume [mount options] -it [dockerhub_username]/novoplasty:[version] ls /path/to/reads/reads_1.fastq` must return that the file exists.
		- In the example above, if `./path/to/reads/reads_1.fastq` is a directory in the current working directory on the host, then the container could be invoked with `docker run --volume "${PWD}":/work NOVOPlasty.pl -c config.txt`. Then, the absolute paths within `config.txt` ought to point to `/work/path/to/reads/reads_1.fastq`.
